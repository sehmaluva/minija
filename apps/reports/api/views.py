...
from apps.reports.api.serializers import (
    ReportSerializer, ReportCreateSerializer, AlertSerializer, AlertUpdateSerializer
)
...
def analytics_dashboard_view(request):
    ...
    egg_analytics = {
        'total_eggs_30_days': recent_egg_production.aggregate(total=Sum('total_eggs'))['total'] or 0,
        'average_production_rate': recent_egg_production.aggregate(avg=Avg('production_rate'))['avg'] or 0,
        'daily_production_trend': [],
        'grade_distribution': {
            'grade_a': recent_egg_production.aggregate(total=Sum('grade_a_eggs'))['total'] or 0,
            'grade_b': recent_egg_production.aggregate(total=Sum('grade_b_eggs'))['total'] or 0,
            'grade_c': recent_egg_production.aggregate(total=Sum('grade_c_eggs'))['total'] or 0,
            'cracked': recent_egg_production.aggregate(total=Sum('cracked_eggs'))['total'] or 0,
            'dirty': recent_egg_production.aggregate(total=Sum('dirty_eggs'))['total'] or 0,
        }
    }
    ...
    feed_analytics = {
        'total_consumption_30_days': recent_feed.aggregate(total=Sum('quantity_kg'))['total'] or 0,
        'total_cost_30_days': sum(record.quantity_kg * record.cost_per_kg for record in recent_feed),
        'feed_by_type': recent_feed.values('feed_type').annotate(
            total_kg=Sum('quantity_kg'),
            avg_cost=Avg('cost_per_kg')
        ).order_by('-total_kg'),
        'top_suppliers': recent_feed.values('supplier').annotate(
            total_kg=Sum('quantity_kg')
        ).order_by('-total_kg')[:5]
    }
    ...
    health_analytics = {
        ...
        'total_mortality_30_days': recent_mortality.aggregate(total=Sum('count'))['total'] or 0,
    ...
    financial_analytics = {
        ...
        'total_operational_costs_30_days': feed_analytics['total_cost_30_days'] + health_analytics['health_cost_30_days'],
        'cost_per_bird_30_days': (feed_analytics['total_cost_30_days'] + health_analytics['health_cost_30_days']) / total_birds if total_birds > 0 else 0
    }
...
def generate_report_view(request):
    ...
    try:
        ...
        if report_type == 'production':
            ...
            report_data = {
                'total_eggs': egg_production.aggregate(total=Sum('total_eggs'))['total'] or 0,
            ...
            report_data = {
                ...
                'vaccinations': health_records.filter(record_type='vaccination').count(),
                ...
                'health_costs': health_records.aggregate(total=Sum('cost'))['total'] or 0,
...
def bulk_alert_update_view(request):
    ...
    if user.role in ['admin']:
        ...
        alerts = Alert.objects.filter(
            Q(farm__owner=user) | Q(farm__managers=user),
            id__in=alert_ids
        )
