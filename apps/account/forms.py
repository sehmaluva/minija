...
from apps.account.models.models import EmailAddress
...
class SignupForm(forms.Form):
    ...
    def clean(self):
        if (
            ...
            and self.cleaned_data["password"] != self.cleaned_data["password_confirm"]
        ):
...
class SettingsForm(forms.Form):
    ...
    if settings.USE_I18N:
        language = forms.ChoiceField(
            label=_("Language"), choices=settings.ACCOUNT_LANGUAGES, required=False
        )
