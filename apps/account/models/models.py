    ...
    def create(cls, request=None, **kwargs):
        ...
        if "language" not in kwargs:
            if request is None:
                ...
                account.language = translation.get_language_from_request(
...
def user_post_save(*args, **kwargs):
    ...
    disabled = getattr(
        user, "_disable_account_creation", not settings.ACCOUNT_CREATE_ON_SAVE
    )
...
class AccountDeletion(models.Model):
    ...
    def expunge(cls, hours_ago=None):
        ...
        count = 0
        for account_deletion in cls.objects.filter(
            date_requested__lt=before, user__isnull=False
        ):
    ...
    @classmethod
    def mark(cls, user):
        account_deletion, created = cls.objects.get_or_create(
            user=user
        )  # skipcq: PYL-W0612
