...
from apps.account.models.models import EmailConfirmation, SignupCode
from apps.account.views import INTERNAL_RESET_URL_TOKEN, PasswordResetTokenView
...
class LoginViewTestCase(TestCase):
    ...
    def test_post_success(self):
        ...
        self.assertRedirects(
            response, settings.ACCOUNT_LOGIN_REDIRECT_URL, fetch_redirect_response=False
        )
...
class ConfirmEmailViewTestCase(TestCase):
    ...
    def test_get_bad_key(self):
        response = self.client.get(
            reverse("account_confirm_email", kwargs={"key": "badkey"})
        )
        self.assertEqual(response.status_code, 404)
...
class ChangePasswordViewTestCase(TestCase):
    ...
    def test_get_anonymous(self):
        ...
        self.assertRedirects(
            response, reverse("account_password_reset"), fetch_redirect_response=False
        )
...
class PasswordResetTokenViewTestCase(TestCase):
    ...
    def test_get_abuse_reset_token(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, PasswordResetTokenView.template_name_fail)
