...
from apps.account.utils.utils import check_password_expired
...
@override_settings(ACCOUNT_PASSWORD_USE_HISTORY=True)
@modify_settings(
    **middleware_kwarg({"append": "account.middleware.ExpiredPasswordMiddleware"})
)
class PasswordExpirationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.history = PasswordHistory.objects.create(
            user=self.user, password=make_password(self.password)
        )
    ...
    def test_get_expired(self):
        ...
        self.history.timestamp = datetime.datetime.now(
            tz=pytz.UTC
        ) - datetime.timedelta(days=1, seconds=self.expiry.expiry)
        self.history.save()
    ...
    def test_password_expiration_reset(self):
        ...
        self.client.post(reverse("account_password"), post_data)
...
class ExistingUserNoHistoryTestCase(TestCase):
    ...
    def test_password_reset(self):
        ...
        with override_settings(ACCOUNT_PASSWORD_USE_HISTORY=False):
            self.client.post(reverse("account_password"), post_data)
