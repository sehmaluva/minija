...
from apps.account.models.models import PasswordExpiry, PasswordHistory


@override_settings(ACCOUNT_PASSWORD_EXPIRY=500)
class UserPasswordExpiryTests(TestCase):
    ...
    def test_set_explicit_password_expiry(self):
        ...
        self.assertIn(
            'User "{}" password expiration set to {} seconds'.format(
                self.user.username, expiration_period
            ),
            out.getvalue(),
        )
    ...
    def test_set_default_password_expiry(self):
        ...
        self.assertFalse(hasattr(self.user, "password_expiry"))
        out = StringIO()
        call_command("user_password_expiry", "patrick", stdout=out)
        ...
        self.assertIn(
            'User "{}" password expiration set to {} seconds'.format(
                self.user.username, default_expiration
            ),
            out.getvalue(),
        )
    ...
    def test_reset_existing_password_expiry(self):
        ...
        """
        previous_expiry = 123
        existing_expiry = PasswordExpiry.objects.create(
            user=self.user, expiry=previous_expiry
        )
        out = StringIO()
        call_command("user_password_expiry", "patrick", stdout=out)
    ...
    def test_bad_username(self):
        ...
        out = StringIO()
        call_command("user_password_expiry", bad_username, stdout=out)
...
class UserPasswordHistoryTests(TestCase):
    ...
    def test_set_history(self):
        ...
        out = StringIO()
        call_command(
            "user_password_history", "--days={}".format(password_age), stdout=out
        )
