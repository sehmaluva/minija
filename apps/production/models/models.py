...
class FeedRecord(models.Model):
    ...
    recorded_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="recorded_feeds"
    )
...
class EggProduction(models.Model):
    ...
    average_weight = models.DecimalField(
        max_digits=5, decimal_places=2, help_text="Average egg weight in grams"
    )
    recorded_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="recorded_egg_productions"
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class WeightRecord(models.Model):
    ...
    average_weight = models.DecimalField(
        max_digits=6, decimal_places=2, help_text="Average weight in grams"
    )
    min_weight = models.DecimalField(
        max_digits=6, decimal_places=2, help_text="Minimum weight in grams"
    )
    max_weight = models.DecimalField(
        max_digits=6, decimal_places=2, help_text="Maximum weight in grams"
    )
    age_in_days = models.PositiveIntegerField()
    notes = models.TextField(blank=True, null=True)
    recorded_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="recorded_weights"
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class EnvironmentalRecord(models.Model):
    ...
    temperature = models.DecimalField(
        max_digits=5, decimal_places=2, help_text="Temperature in Celsius"
    )
    humidity = models.DecimalField(
        max_digits=5, decimal_places=2, help_text="Humidity percentage"
    )
    ...
    lighting_hours = models.DecimalField(
        max_digits=4, decimal_places=2, null=True, blank=True
    )
    notes = models.TextField(blank=True, null=True)
    recorded_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="recorded_environmental"
    )
    created_at = models.DateTimeField(auto_now_add=True)
